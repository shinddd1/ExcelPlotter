...
class PlotSettingsWindow:
    ...
    def _build_y_color_section(self, parent):
        ...
        for i, y_col in enumerate(y_columns):
            ...
            if y_col in self.settings['y_marker_sizes']:
                ...
                marker_size_var.set(self.settings.get('marker_size', 6.0))
    ...
    def _build_numeric_axis_tab(self, parent):
        ...
        ttk.Label(mode_frame, text='X축 범위 모드:').pack(side=tk.LEFT)
        
        self.x_auto_mode = tk.BooleanVar(value=False)
        ttk.Radiobutton(mode_frame, text='수동', variable=self.x_auto_mode, value=False, 
                       command=self._toggle_x_manual_mode).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(mode_frame, text='자동', variable=self.x_auto_mode, value=True, 
                       command=self._toggle_x_auto_mode).pack(side=tk.LEFT, padx=5)
    ...
    def _build_timestamp_axis_tab(self, parent):
        ...
        mode_frame = ttk.Frame(parent)
        mode_frame.pack(fill=tk.X, pady=5)
        ttk.Label(mode_frame, text='X축 시간 범위 모드:').pack(side=tk.LEFT)
        
        self.x_timestamp_auto_mode = tk.BooleanVar(value=False)
        ttk.Radiobutton(mode_frame, text='수동', variable=self.x_timestamp_auto_mode, value=False, 
                       command=self._toggle_x_timestamp_manual_mode).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(mode_frame, text='자동', variable=self.x_timestamp_auto_mode, value=True, 
                       command=self._toggle_x_timestamp_auto_mode).pack(side=tk.LEFT, padx=5)
        ...
        # 초기 상태 설정
        self._toggle_x_timestamp_manual_mode()
    ...
    def _toggle_x_manual_mode(self):
        """숫자 축 수동 모드 활성화"""
        self.xlim_min_entry.config(state='normal')
        self.xlim_max_entry.config(state='normal')
        
    def _toggle_x_auto_mode(self):
        """숫자 축 자동 모드 활성화"""
        self.xlim_min_entry.config(state='disabled')
        self.xlim_max_entry.config(state='disabled')
        # 자동으로 범위 설정
        self._auto_xlim()
        
    def _toggle_x_timestamp_manual_mode(self):
        """시간 축 수동 모드 활성화"""
        self.xlim_min_entry.config(state='normal')
        self.xlim_max_entry.config(state='normal')
        
    def _toggle_x_timestamp_auto_mode(self):
        """시간 축 자동 모드 활성화"""
        self.xlim_min_entry.config(state='disabled')
        self.xlim_max_entry.config(state='disabled')
        # 자동으로 범위 설정
        ...
        try:
            ...
            try:
                if settings.get('is_timestamp', False):
                    ...
                    ax.set_yticks(np.linspace(settings['ylim_min'], settings['ylim_max'], settings['y_ticks']))
                else:
                    # 숫자 축인 경우 (안전한 변환)
                    try:
                        if isinstance(settings['xlim_min'], str):
                    ...
                    ax.set_ylim(settings['ylim_min'], settings['ylim_max'])
                    ax.set_xticks(np.linspace(xlim_min, xlim_max, settings['x_ticks']))
                    ax.set_yticks(np.linspace(settings['ylim_min'], settings['ylim_max'], settings['y_ticks']))
            except Exception as e:
                print(f"축 설정 오류: {e}")
                # 기본값 사용
    ...
    def _collect_settings(self):
        ...
        try:
            ...
            if tab_index == 0:  # 숫자축 탭
                # 숫자축 처리
                if hasattr(self, 'x_auto_mode') and self.x_auto_mode.get():
                    # 자동 모드 - 실제 데이터 범위 사용
                    try:
                        current_sheet = self.main_app.sheet_combo.get()
                        x_col = self.main_app.x_combo.get()
                        if current_sheet and current_sheet in self.main_app.df_map and x_col:
                            df = self.main_app.df_map[current_sheet]
                            x_data = pd.to_numeric(df[x_col], errors='coerce').dropna()
                            if not x_data.empty:
                                settings['xlim_min'] = float(x_data.min())
                                settings['xlim_max'] = float(x_data.max())
                            else:
                ...
                # 시간축 처리
                if hasattr(self, 'x_timestamp_auto_mode') and self.x_timestamp_auto_mode.get():
                    # 자동 모드 - 실제 데이터 범위 사용
                    try:
                        current_sheet = self.main_app.sheet_combo.get()
                        x_col = self.main_app.x_combo.get()
