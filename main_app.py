...
class ExcelPlotterApp:
    ...
    def _refresh_columns(self) -> None:
        ...
        # heuristic: first column as X
        if cols:
            self.x_combo.set(cols[0])
            print(f"X 컬럼 설정: {cols[0]}")
    ...
    def apply_plot_settings(self, settings: dict) -> None:
        ...
        try:
            ...
            if axis_type == 'datetime':
                ...
                # 숫자축인 경우
                x_ticks = np.linspace(settings['xlim_min'], settings['xlim_max'], settings['x_ticks'])
                self.ax.set_xticks(x_ticks)
            
            y_ticks = np.linspace(settings['ylim_min'], settings['ylim_max'], settings['y_ticks'])
